...
** Add `lock`, `unlock` and `setpassword` command to do lock management. If password is not default(""), everytime user open the application,
...
*** Added in `unlock` and `setpassword` command.
