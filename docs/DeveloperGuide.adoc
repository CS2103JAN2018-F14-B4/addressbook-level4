...
An addition class `CipherEngine` is added to the `Bookshelf` class. This class contatins some methods
  ...
  * ** When `Lock` command is entered **
  ...
  . 'LockCommand' will check whether the `password` is `defaultpassword`: "".
  . If the `password` is not `defaultpassword`, `LockCommand` is processed to set `isLock`(`LockManager`) to be true.
  . If the `password` is `defaultpassword`, `LockCommand` will send a logging information to inform user that they cannot lock the `Bookshelf`.
  =====
  NOTE

  . The `Bookshelf` cannot be run any command except `unlock` and `help` command.

  . You cannot Lock the `Bookshelf` if the `password` is `defaultpassword`.
  =====
