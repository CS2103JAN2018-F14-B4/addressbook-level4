...
. `EditCommand` is processed to set isEncrypt to be true.
. The `Biblitek` cannot be run any command except `decrypt` and `help` command.

When the `Biblitek` is encrypted and `decrypt` command is entered,

. `DecryptCommandParser` will extract key from the user input.
. `DecryptCommandParser` creates an `DecryptCommand` the key.
. `DecryptCommand` is preprocessed to check that key is correct.
...
. `setKeyCommand` is processed to post an keyChangedEvent to the eventcenter.
. Modelmanager indicate the keyChangedevent and handle it, and set the bookshelf's key to be new key.
. StorageManager handle the event by update the biblitek with the new key.
