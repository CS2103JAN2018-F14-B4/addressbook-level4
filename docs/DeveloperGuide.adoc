...
You can get the latest Java 8 JDK at http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html[link].
+
...
The rest of the App consists of five components. These components and a brief summary of their responsibilties are given below.
...
The `Logic` component is home to various `Command` s, which may or may not be undoable. The structural relationship between `Command` and `UndoableCommand` is shown in Figure 7 below.

.Structure of Commands in the Logic Component. This diagram shows finer details concerning `XYZCommand` and `Command` in <<fig-LogicClassDiagram>>
image::LogicCommandClassDiagram.png[width="800"]

NOTE: `UndoableCommand` s are commands that cause change to the stored data.
...
Classes used by multiple components are in the `seedu.address.commons` package. This includes Events and Util (utility) classes.
...
Managing these _dependencies_ can be automated using Gradle. Gradle can download the dependencies automatically.
