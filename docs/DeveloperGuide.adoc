...
`Model` exposes a method `addRecentBook(Book)` to handle new book selections. Whenever a new book is selected, `LogicManager` handles the `BookListSelectionChangedEvent`
