...
. If the oldKey is correct, `setKeyCommand` will send a setKey command to LogicManager.
. If the newKey is "", it means that you do not want to encrypt the data and book shelf, then bibliotek will decrypt your data file and your bookshelf.
. If the newKy is not "", LogicManager will send set_new_key request to ModelManger.
