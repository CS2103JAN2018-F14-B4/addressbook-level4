...
Interested? Jump to <<Installing Bibliotek>> to get Bibliotek. Enjoy!
...
The interface of Bibliotek can be broken down to 4 main components.
...
+
image::CommandBox.png[width="450"]
+
...
. The left panel displays a list of books, which is the result of your `search` command. These books are identified by their indexes (given beside the book title). +
+
image::LeftPanel.png[width="90"]
+
...
+
image::RightPanel.png[width="450"]
+
...
=== Command autocorrection system _(since v1.5)_
...
With the command autocorrection system, this may only be a press of kbd:[Enter] away.

****
* Corrects inputed command word which is one letter away from an actual command word, and suggests the correction to you.
** May not correct for additional space in the command word.
* You can execute the corrected command by pressing kbd:[Enter].
****

== Other Features

This section documents features in Bibliotek which are neither book management nor convenience features.
...
// tag::encrypt[]
[[Encrypt-command]]
=== Lock Bibliotek : `lock` _(coming in v1.5)_

If you are going away for a moment and want to prevent others from messing with the application, you can perform `lock`. +
Format: `lock`

****
* Locks Bibliotek. No commands can be performed other than `help` and `unlock`.
* Bibliotek is locked upon start up if you have a password.
****

NOTE: If you perform `lock` without a password, anyone can `unlock` your application.

[[Decrypt-command]]
=== Unlock Bibliotek : `unlock` _(coming in v1.5)_

If you previously performed `lock`, use `unlock` to unlock Bibliotek. +
...
NOTE: If you did not set a password, simply use `unlock` to unlock Bibliotek.
...
*A*: Yes, you are free to build upon Bibliotek. The source code for Bibliotek is available at https://github.com/CS2103JAN2018-F14-B4/main[our github page].

== Command Summary

If all you need is a quick reference on how to use certain commands, you can refer to this list below.
...
|*List books in book shelf* |`list [t/TITLE] [a/AUTHOR] [c/CATEGORY] [s/STATUS] [p/PRIORITY] [r/RATING] [by/SORT_BY]`
|*Lock Bibliotek* |`lock`
...
|*Unlock Bibliotek* |`unlock PASSWORD`
|=======================================================================
