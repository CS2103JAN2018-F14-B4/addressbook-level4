...
// tag::encrypt[]
[[setPassword-command]]
=== Setting a new password to bookshelf : `setpassword` _(coming in v1.5)_

If you want to keep your data secure, you can opt to set a key for bibliotek. +
Format: `setpassword old/OLDPASSWORD new/NEWPASSWORD`
...
If you want to decrypt your bookshelf if your bookshelf is locked, you can lock it using UnlockCommand. +
Format: `unlock YOUR_KEY`

[NOTE]
====
You will be given a default key: "", when you open bibliotek for the first time.
====
...
. Your cannot lock the bookshelf if the key is default key "".
====
