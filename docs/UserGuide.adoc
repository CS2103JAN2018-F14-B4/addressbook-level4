...
* `COMMAND` must refer to a default, built-in command, and can optionally include command parameters.
...
For example, if you want to have a custom `list` command that sorts by title (in reverse alphabetical order) by default,
